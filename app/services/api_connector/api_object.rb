...
module APIConnector
  class APIObject < ServiceObject
    ...
    def delete_request(endpoint, payload = '')
  ...
  def build_http_request(endpoint, payload, http)
    request = http.new("#{api_url}/#{endpoint}",
    ...
    def make_token_request
      client  = OAuth2::Client.new(ENV['COGNITO_USERNAME'],ENV['COGNITO_PASSWORD'], :site => ENV['COGNITO_HOST'], :token_url => '/oauth2/token')
      begin
